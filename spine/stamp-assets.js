...
// the spine's HTML files so browsers re-fetch updated CSS/JS automatically.
//
...
// `?v=...` query string on every <link href> / <script src> in spine/*.html
// to match. One hash for all assets — any change busts everything.
//
// Run: `node spine/stamp-assets.js`. Idempotent.
...
// Files to restamp — every .html in spine/, picked up automatically so new
// pre-rendered section-N.html files get included without a list edit.
const HTML = fs.readdirSync(SPINE).filter(f => f.endsWith('.html')).sort();

const parts = [];
for (const a of ASSETS) {
  const p = path.join(SPINE, a);
